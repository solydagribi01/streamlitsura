...
if df is not None:
    ...
    if opcion == "Análisis":
        ...
        if not df_filtered_analysis.empty:
            ...
            if "denominacion_ejecutante" in df_filtered_analysis.columns and "costes_totreales" in df_filtered_analysis.columns:
                costos_por_ejecutante = df_filtered_analysis.groupby("denominacion_ejecutante")["costes_totreales"].sum().sort_values(ascending=False)
                if not costos_por_ejecutante.empty:
                    fig, ax = plt.subplots(figsize=(10, min(len(costos_por_ejecutante) * 0.5, 10))) # Ajustar altura dinámicamente
                    sns.barplot(x=costos_por_ejecutante.values, y=costos_por_ejecutante.index, palette="viridis", ax=ax)
                    ax.set_xlabel("Costo Total ($)")
                    ax.set_ylabel("Ejecutante")
                    ax.set_title("Costo Total por Ejecutante")
                    st.pyplot(fig)
                else:
                    st.info("No hay datos de costos por ejecutante para mostrar.")
            else:
                st.warning("Columnas 'denominacion_ejecutante' o 'costes_totreales' no encontradas para el gráfico de costos.")

            # --- Indicadores Globales para el DataFrame filtrado ---
            st.subheader("📊 Indicadores Clave del Mantenimiento (Datos Filtrados)")
            
            # Calcular MTTR, MTBF, Disponibilidad para el DataFrame filtrado
            # Estas funciones requieren 'TIEMPO PARADA', 'EQUIPO_NUM', 'AVISO_NUM', 'DIAS/ AÑO', 'HORA/ DIA'
            # Asegúrate de que estas columnas estén presentes o maneja su ausencia.
            
            # Global MTTR (Mean Time To Repair)
            global_mttr = df_filtered_analysis['TIEMPO PARADA'].mean() if 'TIEMPO PARADA' in df_filtered_analysis.columns else np.nan

            # Global MTBF (Mean Time Between Failures)
            # Para MTBF global, necesitamos la suma total de tiempo de operación y fallas.
            # Asumimos que los valores de 'DIAS/ AÑO' y 'HORA/ DIA' son promedios o constantes representativas.
            total_operacion_horas = (df_filtered_analysis['DIAS/ AÑO'].mean() * df_filtered_analysis['HORA/ DIA'].mean() * df_filtered_analysis['EQUIPO_NUM'].nunique() if 'EQUIPO_NUM' in df_filtered_analysis.columns else 0) \
                                     if 'DIAS/ AÑO' in df_filtered_analysis.columns and 'HORA/ DIA' in df_filtered_analysis.columns else np.nan
            total_tiempo_parada = df_filtered_analysis['TIEMPO PARADA'].sum() if 'TIEMPO PARADA' in df_filtered_analysis.columns else 0
            total_fallas = df_filtered_analysis['AVISO_NUM'].count() if 'AVISO_NUM' in df_filtered_analysis.columns else 0

            global_mtbf = (total_operacion_horas - total_tiempo_parada) / total_fallas if total_fallas > 0 and not pd.isna(total_operacion_horas) else np.nan
            
            # Global Disponibilidad
            global_disponibilidad = (global_mtbf / (global_mtbf + global_mttr)) * 100 if not pd.isna(global_mttr) and not pd.isna(global_mtbf) and (global_mtbf + global_mttr) != 0 else np.nan


            col_m1, col_m2, col_m3 = st.columns(3)
            col_m1.metric("MTTR promedio (hrs)", f"{global_mttr:.2f}" if not pd.isna(global_mttr) else "N/A")
            col_m2.metric("MTBF promedio (hrs)", f"{global_mtbf:.2f}" if not pd.isna(global_mtbf) else "N/A")
            col_m3.metric("Disponibilidad (%)", f"{global_disponibilidad:.2f}%" if not pd.isna(global_disponibilidad) else "N/A")
            ...
            st.subheader(f"Tabla de {selected_analysis_option}")
            display_analysis_table(df_filtered_analysis, col_agrup, col_cost, tipo_calc, st.session_state.analysis_page)
        else:
            st.info("No hay datos disponibles para los filtros seleccionados en la sección de Análisis.")
    ...
    # --- EVALUACIÓN ---
        ...
        if df_sub is not None and not df_sub.empty:
            ...
            # --- Sección de Evaluación Cualitativa por Aviso (con Sliders) ---
            st.markdown("---")
            st.subheader("Evaluación Cualitativa Detallada por Aviso")

            if 'aviso' in df_sub.columns and not df_sub['aviso'].empty:
                aviso_seleccionado = st.selectbox("Selecciona un aviso para evaluar cualitativamente", df_sub["aviso"].unique(), key="aviso_eval_slider")
                
                # Asegúrate de que 'aviso_seleccionado' no sea None (puede pasar si no hay avisos)
                if aviso_seleccionado:
                    aviso_data = df_sub[df_sub["aviso"] == aviso_seleccionado]
                    if not aviso_data.empty:
                        aviso_data = aviso_data.iloc[0] # Tomar la primera fila si hay duplicados por aviso

                        st.write("### Detalles del Aviso Seleccionado")
                        # Mostrar detalles relevantes del aviso
                        st.json({
                            "Equipo": aviso_data.get("equipo", "N/A"),
                            "Descripción": aviso_data.get("descripcion", "N/A"),
                            "Duración de parada (hrs)": aviso_data.get("TIEMPO PARADA", "N/A"),
                            "Costo total (pesos)": f"{aviso_data.get('COSTO', 'N/A'):.2f}" if not pd.isna(aviso_data.get('COSTO')) else "N/A",
                            "Tipo de Servicio": aviso_data.get("tipo_de_servicio", "N/A")
                        })

                        st.write("### Evaluación Cualitativa con Preguntas (por Aviso)")

                        # Preguntas específicas para la evaluación por aviso (simplificadas o adaptadas)
                        # Usaremos las preguntas generales pero con sliders
                        # Ajustar el rango del slider de -1 a 2
                        slider_preguntas = [
                            ("Calidad", "¿Las soluciones propuestas son coherentes con el diagnóstico y causa raíz del problema?"),
                            ("Calidad", "¿El trabajo entregado tiene materiales nuevos, originales y de marcas reconocidas?"),
                            ("Calidad", "¿Cuenta con acabados homogéneos, limpios y pulidos?"),
                            ("Oportunidad", "¿Cumple las fechas y horas programadas para los trabajos?"),
                            ("Postventa", "¿Tiene disposición y actitud de servicio frente a solicitudes?"),
                            ("Precio", "¿Los precios ofrecidos son competitivos respecto al mercado?")
                        ]
                        
                        respuestas_slider = []
                        for idx, (area, pregunta_texto) in enumerate(slider_preguntas):
                            # Para un aviso específico, la clave del estado del slider debe ser única
                            slider_key = f"slider_{aviso_seleccionado}_{area}_{idx}"
                            current_value = st.session_state.get(slider_key, 0) # Obtener valor previo o 0 por defecto

                            valor = st.slider(f"**[{area}]** {pregunta_texto}", min_value=-1, max_value=2, value=current_value, step=1, key=slider_key)
                            st.session_state[slider_key] = valor # Guardar el estado del slider
                            respuestas_slider.append(valor)

                        if respuestas_slider:
                            promedio_slider = np.mean(respuestas_slider)
                            st.success(f"Puntaje promedio para el Aviso **{aviso_seleccionado}**: {promedio_slider:.2f} / 2")

                            # --- Mostrar resultados en tabla ---
                            columnas_eval_slider = [f"{p[0]} - {p[1]}" for p in slider_preguntas]
                            # Reducir el texto de la columna para que se vea mejor en la tabla
                            columnas_eval_slider_display = [col.split('] ')[1].replace('¿', '').replace('?', '')[:30] + '...' if len(col.split('] ')[1]) > 30 else col.split('] ')[1].replace('¿', '').replace('?', '') for col in columnas_eval_slider]
                            
                            evaluacion_df_slider = pd.DataFrame({
                                "Aviso": [aviso_seleccionado],
                                **{col_disp: [val] for col_disp, val in zip(columnas_eval_slider_display, respuestas_slider)},
                                "Promedio": [promedio_slider]
                            })

                            st.write("### Resultado de Evaluación por Aviso")
                            st.dataframe(evaluacion_df_slider, use_container_width=True)
                        else:
                            st.info("No hay respuestas de evaluación para este aviso.")
                    else:
                        st.info(f"No se encontraron datos para el aviso {aviso_seleccionado}.")
            else:
                st.info("No hay avisos disponibles para realizar una evaluación cualitativa detallada. Por favor, sube un archivo con datos de avisos.")
